...
Prob_gen::Prob_gen(int number_of_trials, int number_of_states, float given_sigma, float *state_boundary)
{
    ...
    prob_mat = new float[state_cnt*state_cnt]{};
    gen_trans_mat();
}
...
int Prob_gen::gen_trans_mat()
{
    ...
    for (int i = 0; i < state_cnt; ++i)
    {
        ...
        for (int offset = 0; offset < state_cnt/2 + 1; ++offset)
        {
            ...
            int j_l = i - offset;
            if (j_l < 0)
                prob_mat[i*state_cnt+0] += mat_temp[state_cnt/2 - offset];
            else
                prob_mat[i*state_cnt+j_l] = mat_temp[state_cnt/2 - offset];

            // check j = i+1, i+2, ..., state_cnt-1
            int j_r = i + offset;
            if (j_r > state_cnt-1)
                prob_mat[i*state_cnt+(state_cnt-1)] += mat_temp[state_cnt/2 + offset];
            else
                prob_mat[i*state_cnt+j_r] = mat_temp[state_cnt/2 + offset];
        }
    }
    return 0;
}