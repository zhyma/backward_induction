...
DPModel::DPModel(int pred_steps, int running_steps)
{
    ...
    for (int i = 0; i < d.n; ++i)
        d.list[i] = float(i * v.bound[1] * N_pred * dt/(n_t-1));
    d.bound[0] = d.list[0];
    d.bound[1] = d.list[d.n-1];
...
float DPModel::terminal_cost(int dk0, int dk, int vk)
{
    ...
    float d_target = d0x + N_pred * dt * v.bound[1];
    float v_target = v.bound[1];
...
int DPModel::val_to_idx(float val, struct Set *ref)
{
    ...
    int idx = 0;
    idx = round((val - ref->bound[0])/((ref->bound[1]-ref->bound[0])/(ref->n-1)));
